    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-white border-2 border-{{ $analisis['salud_proyecto']['emoji'] == '🟢' ? 'green' : ($analisis['salud_proyecto']['emoji'] == '🟡' ? 'yellow' : ($analisis['salud_proyecto']['emoji'] == '🟠' ? 'orange' : 'red')) }}-200 rounded-xl p-5 shadow-sm">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-sm font-semibold text-gray-700">Salud del Proyecto</span>
                        <span class="text-3xl">{{ $analisis['salud_proyecto']['emoji'] }}</span>
                    </div>
                    <p class="text-3xl font-bold text-gray-900">{{ $analisis['salud_proyecto']['score'] }}<span class="text-lg text-gray-600">/100</span></p>
                ...
                <div class="bg-white border-2 border-blue-200 rounded-xl p-5 shadow-sm">
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $analisis['ruta_critica']['duracion_proyecto'] }}</p>
                    <p class="text-sm font-medium text-gray-700 mt-1">días de duración</p>
                </div>

                <div class="bg-white border-2 border-{{ $analisis['salud_proyecto']['total_desviaciones'] > 0 ? 'red' : 'green' }}-200 rounded-xl p-5 shadow-sm">
                    <div class="flex items-center justify-between mb-2">
                ...
                <div class="bg-white border-2 border-{{ $analisis['salud_proyecto']['total_sobrecarga'] > 0 ? 'orange' : 'green' }}-200 rounded-xl p-5 shadow-sm">
                    <div class="flex items-center justify-between mb-2">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl border-2 border-{{ $desv['tipo'] == 'atraso' ? 'red' : 'yellow' }}-200 bg-{{ $desv['tipo'] == 'atraso' ? 'red' : 'yellow' }}-50">
                        ...
                        <div class="flex-1">
                            ...
                            @if($desv['tipo'] == 'atraso')
                            <p class="text-sm font-medium text-red-700 mt-1">⏱️ {{ $desv['dias_atraso'] }} días de atraso - {{ $desv['impacto'] }}</p>
                            @else
                            <p class="text-sm font-medium text-yellow-700 mt-1">⚠️ {{ $desv['probabilidad_atraso'] }}% de probabilidad de atraso - {{ $desv['dias_restantes'] }} días restantes</p>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl border-2 border-amber-200 bg-amber-50 hover:shadow-md transition">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-4 mt-2 text-xs text-gray-600">
                                <span>📅 {{ $ajuste->created_at->diffForHumans() }}</span>
                                <span>✨ Recupera {{ $ajuste->dias_recuperados }} días</span>
                                <span>👥 Afecta {{ $ajuste->recursos_afectados }} {{ $ajuste->recursos_afectados == 1 ? 'recurso' : 'recursos' }}</span>
                            </div>
                        </div>
                        <div>
