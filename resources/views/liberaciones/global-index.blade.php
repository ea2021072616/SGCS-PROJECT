    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="text-white">
                            ...
                            <p class="text-indigo-100 text-lg">Todas las liberaciones de tus proyectos</p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                <form method="GET" action="{{ route('liberaciones.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="proyecto_id" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <select name="ordenar" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" onchange="this.form.submit()">
                            <option value="recientes" {{ request('ordenar') == 'recientes' ? 'selected' : '' }}>Más recientes</option>
                            <option value="antiguas" {{ request('ordenar') == 'antiguas' ? 'selected' : '' }}>Más antiguas</option>
                            <option value="nombre" {{ request('ordenar') == 'nombre' ? 'selected' : '' }}>Por nombre</option>
                            <option value="proyecto" {{ request('ordenar') == 'proyecto' ? 'selected' : '' }}>Por proyecto</option>
                        </select>
                    </div>
                </form>
            </div>
            ...
            @else
                ...
                <div class="space-y-6">
                    @foreach($liberacionesAgrupadas as $proyectoId => $liberacionesProyecto)
                        @php
                            $proyecto = $liberacionesProyecto->first()->proyecto;
                        @endphp
                        ...
                        <div class="bg-white rounded-xl shadow-sm border-2 border-gray-200 overflow-hidden">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="text-white">
                                        ...
                                        <p class="text-xs text-indigo-100">
                                            {{ $proyecto->codigo }} • {{ $liberacionesProyecto->count() }} {{ Str::plural('liberación', $liberacionesProyecto->count()) }}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <!-- Liberaciones del Proyecto (Lista) -->
                            <div class="divide-y divide-gray-200">
                                @foreach($liberacionesProyecto as $liberacion)
                                    <a href="{{ route('proyectos.liberaciones.show', ['proyecto' => $proyecto->id, 'liberacion' => $liberacion->id]) }}"
                                       class="block px-6 py-4 hover:bg-indigo-50 transition-colors group">
                                        <div class="flex items-center justify-between">
                                            <!-- Info de la liberación -->
                                            <div class="flex items-center gap-4 flex-1">
                                                <div class="inline-flex items-center px-2.5 py-1 bg-indigo-600 group-hover:bg-indigo-700 text-white text-xs font-bold rounded-full transition">
                                                    {{ $liberacion->etiqueta }}
                                                </div>
                                                <div class="flex-1">
                                            ...
                                            <div class="flex items-center gap-6 text-sm text-gray-600">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <span class="font-semibold text-indigo-600">{{ $liberacion->items->count() }}</span>
                                                </div>
